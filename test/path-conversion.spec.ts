...
describe('convertFastifyToOpenAPIPath', () => {
  ...
  test('should convert simple path parameters', () => {
    ...
    expect(convertFastifyToOpenAPIPath('/api/v1/products/:productId/reviews/:reviewId')).toEqual({
  ...
  test('should convert regex patterns to OpenAPI style parameters', () => {
    expect(convertFastifyToOpenAPIPath('/user/:action(profile|settings)')).toEqual({
      url: '/user/{action}',
      paramPatterns: { action: 'profile|settings' }
    });
    expect(convertFastifyToOpenAPIPath('/api/v:version(\\d+)/users/:userId')).toEqual({
  ...
  test('should handle multiple regex patterns in a single path', () => {
    expect(convertFastifyToOpenAPIPath('/api/:apiVersion(v1|v2)/:resource/:id(\\d+)-:status([a-z]+)')).toEqual({
  ...
  test('should handle a mix of regex patterns and simple parameters', () => {
    expect(convertFastifyToOpenAPIPath('/:resource/:id(\\d+)-:status/:category')).toEqual({
  ...
  test('should handle edge cases', () => {
    ...
    expect(convertFastifyToOpenAPIPath(':userType(admin|user)/:role/dashboard')).toEqual({
  ...
  test('should handle paths with special characters in regex', () => {
    expect(convertFastifyToOpenAPIPath('/user/:username([a-zA-Z0-9_-]+)')).toEqual({
      url: '/user/{username}',
