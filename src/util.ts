...
export function isTruthy<T extends Exclude<any, Falsy>>(item: T | null | undefined): item is Exclude<T, Falsy> {
  return !isFalsy(item);
}
...
export function isNotReferenceObject<T, U extends T | ReferenceObject = T | ReferenceObject>(item: U): item is Exclude<U, ReferenceObject> {
