...
export const oas3Plugin: FastifyPluginAsync<OAS3PluginOptions> = fp(async (
  fastify,
  options
) => {
  ...
  fastify.addHook('onReady', async () => {
    try {
      ...
      for (const route of routes) {
        const rLog = pLog.child({ route: { url: route.url, method: route.method }});
        ...
        if (route.schema) {
          ...
          if (response) {
            for (const responseCode of Object.keys(response)) {
              if (responseCode !== 'default' && !/^[1-5][0-9]{2}/.test(responseCode.toString())) {
                rLog.warn(`Route has a response schema of code '${responseCode}', which OAS3Plugin does not support.`);
                continue;
              }

              operation.responses[responseCode] = {
                description: oas?.responses?.[responseCode]?.description ?? "No description given.",
                content: {
        ...
        [route.method].flat().forEach(method => p[method.toLowerCase()] = operation);
